...
def create_and_load_iris_(conn, iris_file: Path):
...
def create_and_load_types_(conn, types_data: list[dict]):
...
def drop_table(
    table_name: str,
    conn: .Connection | sqlalchemy.engine.Engine | sqlalchemy.engine.Connection,
):
    if isinstance(conn, .Connection):
...
def sqlite_adbc_iris(sqlite_adbc_conn, iris_path):
    ...
    try:
        ...
        conn.rollback()
        create_and_load_iris_(conn, iris_path)
    try:
        conn.adbc_get_table_schema("iris_view")
    except mgr.ProgrammingError:
        conn.rollback()
...
def sqlite_adbc_types(sqlite_adbc_conn, types_data):
    ...
    try:
        ...
        create_and_load_types_(conn, new_data)
        conn.commit()
...
@pytest.fixture
def sqlite_buildin():
    with contextlib.closing(.connect(":memory:")) as closing_conn:
...
def sqlite_buildin_iris(sqlite_buildin, iris_path):
    create_and_load_iris_(sqlite_buildin, iris_path)
...
def sqlite_buildin_types(sqlite_buildin, types_data):
    ...
    create_and_load_types_(sqlite_buildin, types_data)
    yield sqlite_buildin
...
def sqlite_builtin_detect_types():
    with contextlib.closing(
        .connect(":memory:", detect_types=.PARSE_DECLTYPES)
    ) as closing_conn:
        with closing_conn as conn:
            yield conn
...
def test_xsqlite_execute_closed_connection():
    ...
    """
    with contextlib.closing(.connect(":memory:")) as conn:
        cur = conn.cursor()
        cur.execute(create_sql)
    ...
    with pytest.raises(.ProgrammingError, match=msg):
