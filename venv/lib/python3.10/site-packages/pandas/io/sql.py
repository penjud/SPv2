...
def execute(sql, con, params=None):
    ...
    con : SQLAlchemy connection or  connection
        If a DBAPI2 object, only  is supported.
...
def read_sql_query(
    ...
    sql : str SQL query or SQLAlchemy Selectable (select or text object)
        SQL query to be executed.
    con : SQLAlchemy connectable, str, or  connection
        Using SQLAlchemy makes it possible to use any DB supported by that
        library. If a DBAPI2 object, only  is supported.
    index_col : str or list of str, optional, default: None
        Column(s) to set as index(MultiIndex).
    coerce_float : bool, default True
        Attempts to convert values of non-string, non-numeric objects (like
...
def read_sql(
    ...
    con : ADBC Connection, SQLAlchemy connectable, str, or  connection
        ADBC provides high performance I/O with native type support, where available.
        Using SQLAlchemy makes it possible to use any DB supported by that
        library. If a DBAPI2 object, only  is supported. The user is responsible
...
def to_sql(
    ...
    **engine_kwargs,
    ...
    dtype : dict or scalar, optional
        ...
        SQLAlchemy types or strings for the  fallback mode. If a
    ...
    **engine_kwargs
    ...
    Notes
    -----
    The returned rows affected is the sum of the ``rowcount`` attribute of ``.Cursor``
    ...
    ` <https://docs.python.org/3/library/.html#.Cursor.rowcount>`__ or
...
def pandasSQL_builder(
    ...
    if isinstance(con, .Connection) or con is None:
    ...
    warnings.warn(
        ...
        "database string URI or  DBAPI2 connection. Other DBAPI2 "
...
class SQLiteTable(SQLTable):
    ...
    def _register_date_adapters(self) -> None:
        ...
        # xref https://docs.python.org/3.12/library/.html#adapter-and-converter-recipes
        ...
        .register_adapter(time, _adapt_time)
...
def get_schema(
    ...
    con: ADBC Connection, SQLAlchemy connectable,  connection, default: None
        ADBC provides high performance I/O with native type support, where available.
        Using SQLAlchemy makes it possible to use any DB supported by that
        library
        If a DBAPI2 object, only  is supported.
    dtype : dict of column name to SQL type, default None
        Optional specifying the datatype for columns. The SQL type should
        be a SQLAlchemy type, or a string for  fallback connection.
    schema: str, default: None
